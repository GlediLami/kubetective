...
4. **Score.** Margin = Σ weight·strength − gap penalties. Confidence = sigmoid
   (margin / T). T is fitted against the scenario suite and leave-one-out
   validated, but adopted only if the fit clears three conditions: the suite
   contains incorrect predictions, the fit is not pinned to the edge of the
   search grid, and it beats the default out-of-sample. It currently does not,
   so the engine runs at its hand-set default. Every score carries its
